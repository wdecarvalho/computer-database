...
public class CompanyDao extends Dao<Company> {
	...
	public Optional<Company> find(final Long id) {
		...
		try (final PreparedStatement preparedStatement = this.getConnection().prepareStatement(FIND_ONE_COMPANY,
				ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY)) {
			preparedStatement.setInt(1, id.intValue());
			try (final ResultSet resultSet = preparedStatement.executeQuery()) {
				if (resultSet.next()) {
					company = Optional.ofNullable(new Company.Builder(resultSet.getLong("id")).name(resultSet.getString("name")).build());
				}
			}
	...
	public Collection<Company> findAll() {
		...
		try (final PreparedStatement preparedStatement = this.getConnection().prepareStatement(FIND_ALL_COMPANY,
				ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
	...
	public Pages<Company> findPerPage(int page) {
		...
		try {
			...
			try (final PreparedStatement preparedStatement = this.getConnection().prepareStatement(FIND_COMPUTER_PAGE,
				...
				try (final ResultSet resultSet = preparedStatement.executeQuery()) {
					while (resultSet.next()) {
						pages.getEntities().add(new Company.Builder(resultSet.getLong("id")).name(resultSet.getString("name")).build());
